...
namespace NEL_Wallet_API.lib
{
    ...
    public class TransHelper
    {
        ...
        {
            MyJson.JsonNode_Object response = (MyJson.JsonNode_Object)MyJson.Parse(await TransHelper.HttpGet(api + "?method=getutxo&id=1&params=['" + _addr + "']"));
        ...
        public static ThinNeo.Transaction makeTran(List<Utxo> utxos, List<string> targetaddrs, ThinNeo.Hash256 assetid, decimal sendcount)
        {
            int cnt = targetaddrs.Count();
            var tran = new ThinNeo.Transaction();
            tran.type = ThinNeo.TransactionType.ContractTransaction;
            tran.version = 0;//0 or 1
            tran.extdata = null;
            ...
            for (var i = 0; i < utxos.Count; i++)
            {
                ...
                count += utxos[i].value;
                scraddr = utxos[i].addr;
                if (count >= sendcount * cnt)
                {
                    break;
                }
            }
            tran.inputs = list_inputs.ToArray();
            if (count >= sendcount * cnt)//输入大于等于输出
            {
                List<ThinNeo.TransactionOutput> list_outputs = new List<ThinNeo.TransactionOutput>();
                //输出
                if (sendcount > decimal.Zero && targetaddrs != null)
                {
                ...
                //找零
                var change = count - sendcount*cnt;
                if (change > decimal.Zero)
                {
                    ThinNeo.TransactionOutput outputchange = new ThinNeo.TransactionOutput();
                    outputchange.toAddress = ThinNeo.Helper.GetPublicKeyHashFromAddress(scraddr);
    ...
    {
        public byte[] prikey { set; get; }
        public byte[] pubkey { set; get; }
        public string address { set; get; }

        public static AccountInfo getAccountInfoFromWif(string prikeyWif)
        {
           
            byte[] tmp_prikey = ThinNeo.Helper.GetPrivateKeyFromWIF(prikeyWif);
            byte[] tmp_pubkey = ThinNeo.Helper.GetPublicKeyFromPrivateKey(tmp_prikey);
            string tmp_address = ThinNeo.Helper.GetAddressFromPublicKey(tmp_pubkey);
